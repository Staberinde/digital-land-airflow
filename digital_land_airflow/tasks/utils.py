...
def get_api_instance(kwargs, dataset_name=None):
    if not dataset_name:
...
def get_collection_repository_path(kwargs):
    return Path(kwargs["ti"].xcom_pull(key="collection_repository_path"))


def get_collection_name(kwargs):
    return kwargs["dag"]._dag_id


def get_repo_name(kwargs):
    return f"{get_collection_name(kwargs)}-collection"


get_resources_name = get_repo_name
...
def get_run_temporary_directory(kwargs):
    collection_name = get_collection_name(kwargs)
    run_id = kwargs["run_id"]
    return get_temporary_directory().joinpath(f"{collection_name}_{run_id}")
...
def filter_specified_resources(kwargs, resource_list):
...
def get_pipeline_resource_mapping(kwargs):
    ...
    return {
        pipeline: filter_specified_resources(kwargs, resources)
        for pipeline, resources in pipeline_resource_mapping.items()
    }


def get_resource_pipeline_mapping(kwargs):
...
def upload_directory_to_s3(directory: Path, destination: str):
    if directory.exists():
        files = directory.iterdir()
        upload_files_to_s3(files, destination)
    else:
        logging.warning(
...
def upload_files_to_s3(files, destination):
...
def get_organisation_csv(kwargs):
    if (
    ...
    directory = get_run_temporary_directory(kwargs)
